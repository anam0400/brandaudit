...
<script>
    ...
    {
        loads();
        $.get("<?=base_url()?>admin/relawan/getData", '', function(data){
    ...
    function addData()
    {
        ...
        $.post("<?=base_url()?>admin/relawan/tambahData", formData, function(data){
            data = JSON.parse(data);
            if (data.succ){
                $('#totalMembers').val(1);
                getData();
    ...
    {
        loads();
        $.get("<?=base_url()?>admin/relawan/updateData", '', function(data){
            data = JSON.parse(data);
            if (data.succ){
                $('#formData').html(data.data);
            }else{
