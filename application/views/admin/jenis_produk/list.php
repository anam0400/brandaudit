...
<script>
    ...
    {
        loads();
        $.get("<?=base_url()?>admin/jenis_produk/getData", '', function(data){
    ...
    function addData()
    {
        ...
        $.post("<?=base_url()?>admin/jenis_produk/tambahData", formData, function(data){
            data = JSON.parse(data);
            if (data.succ){
                $('#totalMembers').val(1);
                getData();
    ...
    {
        loads();
        $.get("<?=base_url()?>admin/jenis_produk/updateData", '', function(data){
            data = JSON.parse(data);
            if (data.succ){
                $('#formData').html(data.data);
            }else{
