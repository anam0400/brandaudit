...
<div class="card">
    ...
    <!-- /.card-header -->
    <div class="card-body">
        <form id="form1" method="POST" action="<?=base_url()?>admin/instansi/editProccess/<?=$id?>">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="inputnama">Nama Instansi <span class="text-sm text-danger">*</span></label>
                        <input type="text" id="inputnama" class="form-control form-control-sm" name="nama_instansi" value="<?=$data->data->result->nama_instansi?>" required>
                    </div>
                    <div class="form-group">
                        ...
                        <?php for($i = 0; $i < count($arr); $i++){ ?>
                            <option value="<?=$arr[$i]?>" <?=($arr[$i] == $data->data->result->jenis_instansi)?'selected':''?>><?=ucwords($arr[$i])?></option>
                        <?php } ?>
                        </select>
