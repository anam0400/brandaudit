<div class="card">
    <div class="card-header">
        ...
        <span class="right" style="float: right">
            <a href="<?=base_url()?>admin/instansi/add">
...
<script>
    ...
    {
        loads();
        $.get("<?=base_url()?>admin/instansi/getData", '', function(data){
    ...
    function addData()
    {
        ...
        $.post("<?=base_url()?>admin/instansi/tambahData", formData, function(data){
            data = JSON.parse(data);
            if (data.succ){
                $('#totalMembers').val(1);
                getData();
    ...
    {
        loads();
        $.get("<?=base_url()?>admin/instansi/updateData", '', function(data){
            data = JSON.parse(data);
            if (data.succ){
                $('#formData').html(data.data);
            }else{
